...
**刷寫——不需要 Keil、不需要 toolchain。** repo 內附 **v3.1.23-herdr**，
用固定的 ARM GNU Toolchain 13.2.1 建置。主機回歸測試涵蓋 profile 控制權、
RGB/OLED 前景繪製、連續 NeoPixel 輸出、Herdr 進入時不把 profile 底色畫上
空 agent 鍵、SD 排他、切換、第 15 鍵 script-vs-F9 及 F9 釋放；這不等於
實機 LED 波形驗證。

```bash
dfu-util --device 0483:df11 -a 0 -D ../firmware/duckypad_v3.1.23-herdr.dfu
```

跑起來後，OLED boot 畫面會顯示 `duckyPad V3.1.23`。完整步驟（截圖、
...
韌體 3.1.23+ **需要 microSD 上有 Herdr profile，並且選中它**。
