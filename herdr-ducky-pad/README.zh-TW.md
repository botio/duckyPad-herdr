...
- **Herdr 是可選的 profile，不再自動接管整台 pad。** 韌體 3.1.19+ 只在選到 Herdr profile 時接受 Bridge 的畫面與燈色；**+ / −** 照常切換 profile。
...
**刷寫——不需要 Keil、不需要 toolchain。** repo 內附 **v3.1.19-herdr**，
...
跑起來後，OLED boot 畫面會顯示 `duckyPad V3.1.19`。完整步驟（截圖、
...
韌體 3.1.19+ **需要 microSD 上有 Herdr profile，並且選中它**。
