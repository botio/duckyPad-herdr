...
- **Herdr is a selectable profile**, not a whole-device takeover. Firmware 3.1.17+ only accepts Bridge display data while that profile is selected; **+ / −** still switches profiles.
...
**Flash it — no Keil, no toolchain needed.** The pre-built **v3.1.17-herdr**
...
The OLED boot screen shows `duckyPad V3.1.17` once it's running. The full
...
Firmware 3.1.17+ **requires a microSD card with a selected Herdr profile**.
