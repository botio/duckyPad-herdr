...
**Flash it — no Keil, no toolchain needed.** The pre-built **v3.1.20-herdr**
...
file-access exclusion, navigation, key 15 script-vs-F9 fallback, and F9
release. These do not verify LED waveforms on hardware.

```bash
dfu-util --device 0483:df11 -a 0 -D ../firmware/duckypad_v3.1.20-herdr.dfu
```

The OLED boot screen shows `duckyPad V3.1.20` once it's running. The full
...
Firmware 3.1.20+ **requires a microSD card with a selected Herdr profile**.
